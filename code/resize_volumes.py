...
from Clasification.test1 import *
def resize_data(data, msk):
    ...
    for x, y, z in itertools.product(range(new_size_x),
                                     ...
                                     range(new_size_z)):
        if msk:
            new_data[x][y][z] = round(data[int(x * delta_x)][int(y * delta_y)][int(z * delta_z)])
            if new_data[x][y][z] not in [0,1,2,3,4]:
...
def resize(i):
    ...
    resized_t1 = resize_data(initial_t1, False)
    resized_msk = resize_data(initial_msk, True)
    """
    resized_t1 = process_scan(t1)
    resized_msk = process_scan(msk)
    """
    img_t1 = nib.Nifti1Image(resized_t1, np.eye(4))
    img_msk = nib.Nifti1Image(resized_msk, np.eye(4))
    
    img_t1.to_filename(out_file + "\BraTS20_Training_{}_t1.nii.gz".format(num))
    img_msk.to_filename(out_file + "\BraTS20_Training_{}_seg.nii.gz".format(num))
